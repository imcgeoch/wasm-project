...
||| Only a single kind of value
data Val = I32 Integer

||| Stacks are lists of values
Stack : Type
Stack = List Val

mutual
    ||| There are three types of instructions: addition, if, and const
    data Instr = I32Add | If Expr Expr | Const Val

    ||| Exprs are sequences of instructions
    Expr : Type
    Expr = List Instr

||| The Code type represents a point in time of execution. This comprises a
||| sequence of instructions (the Expr) and an operand stack (the Stack)
...
--------------------------------------------------------------------------------
-----                      LANGUAGE TYPING DEFINITION                      -----
--------------------------------------------------------------------------------

||| There is only a single type in our language
data Tp = T32

||| CodeTp is a list of Tps
CodeTp : Type
CodeTp = List Tp

||| 'Forget' the value and return the type
total
type' : Val -> Tp
type' (I32 x) = T32
...
||| step is the one step reduction function for for our simple language---an
||| interpreter would compute the transitive closure of this function on an
||| input (if it exists)
total
